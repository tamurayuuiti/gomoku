...
// AI探索・評価・キャッシュ・チューニングに関する定数とFeature Flagを定義する。
//
// 方針:
// - 探索強度・評価関数・Worker通信の意味を変えず、設定の責務を明確化する。
// - 本番機能 / 実験機能 / inactive 設定をコメントで区別する。
// - Phase由来の命名は使用せず、現在の役割で管理する。
// - 診断・デバッグ専用設定（旧 DIAGNOSTICS_CONFIG / TIMING_DIAGNOSTICS_CONFIG /
//   state audit / verbose log flag）は diagnosticsFlags.ts へ移動済み。
//   constants.ts からは参照・再exportしない。
...
export const SEARCH_TUNING_CONFIG: {
  ...
  // PVS null-mode policy が有効な場合のみ使用（experimental / 既定OFF）。
  PVS_NULL_MODE: 'quiet_only',
  TIME_PREDICTION_SAFETY: 1.6,
  TIME_PREDICTION_MIN_DEPTH: 3,
  // Conditional root PVS が有効な場合のみ使用（experimental / 既定OFF）。
