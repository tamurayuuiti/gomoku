...
// --- 第8.1弾：feature flags / config ---
/**
 * 第8.1弾で追加する戦術 Quiescence 関連の機能フラグ。
 *
 * 既存の評価値・tier・LMR / PVS / TT の意味は変更しない。
 * 葉ノードでのみ実行される proof 型戦術読み伸ばしである。
 */
export const PHASE8_FEATURES = {
  /** Quiescence 全体 master（検証完了後に true へ切り替え） */
  ENABLE_QSEARCH: true,
  /** qsearch 結果キャッシュを有効化する */
  ENABLE_QSEARCH_CACHE: true,
  /** qsearch 禁手キャッシュを有効化する */
  ENABLE_QSEARCH_FORBIDDEN_CACHE: true,
  /** 負け証明（受け不能 / 強制ブロック後相手勝ち）を有効化する */
  ENABLE_QSEARCH_LOSS_PROOF: true,
  /** qsearch 状態監査（undo 後 stone count 確認） */
...
/**
 * 第8.1弾の設定値。
 * Quiescence の時間・ノード・ply・skip 条件を制御する。
 */
export const PHASE8_CONFIG = {
  /** qsearch 世代（診断・キャッシュ用） */
  QSEARCH_VERSION: 1n,
  /** qsearch 最大 ply */
  QSEARCH_MAX_PLY: 4,
  /** qsearch 1葉あたりノード上限 */
  ...
  /** timeLimitMs がこの値未満なら qsearch を skip（明示 budget なしの場合） */
