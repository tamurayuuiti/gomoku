...
// 第5弾：Static Eval Cache（葉評価キャッシュ）を管理するモジュール。
//
// 責務:
...
//   - 将来の動的禁手や評価関数変更に備え、salt / version を持つ
//
// 第5.5弾:
//   - eviction 統計を「イベント数」ではなく「削除エントリ数」として記録する。
//   - これにより secEvict の意味を直感的にし、統計不整合の追跡を容易にする。
 ...
 * 第5弾では思考単位キャッシュのため、厳密な動的禁手整合は不要だが、
